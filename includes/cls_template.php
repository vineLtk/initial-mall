    ...
    function fetch_str($source)
    {
         ...
         if(preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $source, $sp_match))
        {
             ...
             for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp < $for_max2; $curr_sp++)
            {
                 ...
                 $source= str_replace('%%%SMARTYSP'.$curr_sp.'%%%', str_replace("'", "\'", $sp_match[1][$curr_sp])." ", $source);
            }
         }
...
//            }
    ...
    function get_val($val)
    {
        ...
        {
            $val = preg_replace_callback("/\[([^\[\]]*)\]/is",function($r){return str_replace('$','\$',$r[1]);}, $val);//kk $val = preg_replace("/\[([^\[\]]*)\]/eis", "'.'.str_replace('$','\$','\\1')", $val);
        }
